		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Qmnum") === oObject.Qmnum;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Qmnum;
									} else {
										sCurrentId = aItemsNotInTheList[0].Qmnum;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Qmnum"),
						title: oBindingContext.getProperty("Qmnum")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "pmNotificationSet (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'pmNotificationSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Qmnum");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all pmNotificationSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "pmNotificationSet (" + iExpectedLength + ")"
								}),
								success: function () {
